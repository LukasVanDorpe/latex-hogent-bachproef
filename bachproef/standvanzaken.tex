...
De hardware bestaat uit mechanische onderdelen zoals een deurslot \ref{fig:lockerSlot}, het volledige frame van de unit maar ook uit elektronische onderdelen zoals een camera, Raspberry Pi, digitaal scherm, etc. De mobiele applicatie waar de QR-codes kunnen verkocht worden en zo ook genereren, heeft geen verband met de interne werking van een QR-unit. Het enige dat de QR-unit nodig heeft, is een correct opgebouwde QR-code. Hiermee krijgt de festivalganger toegang tot zijn/haar gehuurde locker.

Elke unit bevat 128 lockers die verspreid zijn over 4 kasten zoals te zien op figuur \ref{fig:lockerUnit}. De locker units zijn waterdicht en dus optimaal voor plaatsing in niet overdekte locaties. Eén lockerkast bevat 24 lockers, 4 grote die zich op de bovenste rij bevinden. De 20 overige zijn een iets kleiner formaat en situeren zich onderaan. De lange zijden van een QR-unit zijn uitklapbare luiken met aan elke kant twee kasten. Er zijn 4 kasten per unit aanwezig wat een totaal geeft van 128 lockers voorgesteld op figuur \ref{fig:lockerUnit} en  \ref{fig:lockerUnit2}.

\begin{figure}[h]
    \centering
    \includegraphics[height=7cm]{F1_lockerSlot.jpg}
    \captionsetup{justification=centering, singlelinecheck=false}    
    \caption{Een locker slot gebruikt om de lockers te sluiten.}
...
\begin{figure}[h]
    \centering
    \includegraphics[height=7cm]{F3_lockerUnit2.jpg}
    \captionsetup{justification=centering, singlelinecheck=false}    
    \caption{Een uitgetekende weergave van een locker unit \autocite{LockitRentals}.}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{F4_voorstellingQrCode.jpg}
    \captionsetup{justification=centering, singlelinecheck=false}    
    \caption{Een technische voorstelling van een QR-code \autocite{Li2018}.}
...
\begin{lstlisting}[caption={Ophalen de zopas aangemaakte gebruiker zijn gegevens indien hij in het huur proces terecht gekomen is \autocite{LockitRentals}.}, label={lst:ophalenUser}]
    /**
...
\begin{lstlisting}[caption={Het informatie ophalen van de geselecteerde locatie. Aan de hand van geselecteerde evenement kan er gegevens uit de databank gehaald worden \autocite{LockitRentals}.}, label={lst:ophalenEventInfo}]
    /**
...
\begin{lstlisting}[caption={De creatie van een reservatie voor het huren van een locker. Deze reservatie wordt bijgehouden in de databank \autocite{LockitRentals}.}, label={lst:aanmakenReservatie}]
    /**
...
De logistieke medewerkers kunnen huurders helpen indien hun QR-code te hard beschadigd is of als ze hun toegangscode tot de locker verloren zijn. Bij het aanmaken van toegangscodes bij logistieke medewerkers verloopt het proces op een alternatieve wijze.  Deze springen over het betalingsproces en verwerven direct een geldige QR-code. Zonder een aangemaakt betalingsveld in de databank op te slaan. Deze toelichting situeert zich in listing \ref{lst:aanmakenBetaling}.

\begin{lstlisting}[caption={Valideert of er een betaling van toepassing is. Indien nodig creëert het een betaling en bewaard deze in de databank \autocite{LockitRentals}.}, label={lst:aanmakenBetaling}]
    /**
...
\begin{lstlisting}[caption={Mollie betaling aanmaken met nodige data \autocite{LockitRentals}.}, label={lst:aanmakenBetalingMollie}]
...
\begin{lstlisting}[caption={Voltooide betaling roept de creatie QR-code methode op. Hierbij gebeurt een status update van de vooraf aangemaakte reservatie \autocite{LockitRentals}.}, label={lst:voltooidebetaling}]
...
\begin{figure}[h]
    ...
    \captionsetup{justification=centering, singlelinecheck=false}    
    \caption{Een voorbeeld van een QR-code die gekocht is door een gebruiker voor specifieke lockertoegang op een evenement \autocite{Dorpe2023}.}
    \label{fig:qrCodeGebruiker}
\end{figure}}

\begin{figure}[h]
    \centering
    \includegraphics[width=0.35\textwidth]{F6_qrcodeAdmin.jpg}
    \captionsetup{justification=centering, singlelinecheck=false}    
    \caption{Een voorbeeld van een QR-code die gegeneerd is door een logistieke medewerker met de juiste bevoegdheden voor specifieke lockertoegang op een evenement \autocite{Dorpe2023}.}
...
\begin{lstlisting}[caption={Typescript code voor een visuele QR-code aan te maken \autocite{LockitRentals}.}, label={lst:qrComponent}]
    await db.runTransaction(async (transaction) => {
...
Lijn 3 \ref{lst:qrComponent}: De variabele ‘activeQrCode’ van type string is de publieke sleutel die wordt gebruikt om de QR-code te ontcijferen. \\
Lijn 10 – 18 \ref{lst:qrComponent}: De ingestelde eigenschappen die gedefinieerd zijn om de QR-code aan te maken. Men kan de code naar zijn behoefte configureren \autocite{Sutheebanjard2010}. Een belangrijke waarde is hierbij de ‘ErrorCorrectionLevel’, deze verandert doorheen de applicatie op basis waar de QR-code afgebeeld moet worden. Hierdoor mag de QR-code voor 30\% beschadigd of gescheurd zijn. De scanner zal alsnog de afbeelding succesvol kunnen detecteren \autocite{Tiwari2016}. \\
Lijn 29– 35 \ref{lst:qrComponent}: De tool die gehanteerd wordt om de QR-code te genereren is de bibliotheek ‘qrcode’ \autocite{pypi2023}. 
...
Om na te gaan of dit bericht zijn oorspronkelijke data bevat zal de functie ‘verify’ opgeroepen worden. In het geval dat dit bericht zich niet in oorspronkelijke status bevindt, zal de functie een foutboodschap retourneren \ref{fig:verifyFailed}. Als de methode geen foutboodschap weergeeft, kan men concluderen dat deze data origineel is \autocite{Bernstein2014}. Zoals wordt weergegeven in figuur \ref{fig:verified}.
...
\begin{lstlisting}[caption={Data object met de naam smallItem bevat de essentiele data om een locker te openen \autocite{LockitRentals}.}, label={lst:essentieleQrdata}]
...
Lijn 11 - 15 \ref{lst:essentieleQrdata} :  De data die essentieel is om toegang te hebben tot de locker wordt gedefinieerd als de variabele ‘smallItem’. Aangezien de eigenschappen zoals kleur, locatie en afbeelding niets te maken hebben met de scanfunctie van de QR-unit wordt deze informatie hiervan gescheiden.
...
\begin{lstlisting}[caption={Digitale ondertekening van QR-code data \autocite{LockitRentals}.}, label={lst:encryptionCode}]
...
\begin{lstlisting}[caption={Samenstellen van object, locker informatie en verificatie code \autocite{LockitRentals}.}, label={lst:createCode}]
...
\begin{lstlisting}[caption={De manier hoe de link van de QR-code is gestructureerd \autocite{LockitRentals}.}, label={lst:createQRLink}]
...
Voor eenvoud van het illustratief voorbeeld vormen we dit om naar een gestandaardiseerd \ac{JSON} object \autocite{Concept}. Het geformatteerd \ac{JSON} object geeft een visueel beeld van de twee variabelen in de genereerde QR-code. Het visueel beeld wordt afgedrukt in de figuur \ref{fig:formattedJsonData}.
...
\begin{lstlisting}[caption={De controle van QR-code data met de TweetNaCL.js verifieer functie \autocite{LockitRentals}.}, label={lst:verifyMethode}]
...
\begin{figure}[h]
    ...
    \captionsetup{justification=centering, singlelinecheck=false}
    \caption{Het vooraanzicht van een QR-unit van Lockit Rentals met camera en digitaal scherm.}
    \label{fig:zijkantLocker}
\end{figure}

\begin{figure}[h]
    \centering
    \includegraphics[angle=-90, width=0.4\textwidth]{F21_LockerUSBCamera.jpeg}
    \captionsetup{justification=centering, singlelinecheck=false}
    \caption{De USB camera ingebouwd inclusief verlichting in een QR-unit van Lockit Rentals.}
...
\begin{figure}[h]
    \centering
    \includegraphics[angle=90, width=\textwidth]{F16_schemaWerking.jpg}
...
De QR-code die alvorens is aangemaakt wordt uitgelezen met een USB camera. De software van de camera draait rechtstreeks op de Rasberry PI. 
 \begin{lstlisting}[language=Python, caption={Het uitvoerbare script van de QR-unit, die de toegangscode zal uitlezen \autocite{LockitRentals}.}, label={lst:pythonPrimary1}, numbers=left]
...
Een Raspberry PI is een kleine maar krachtige mini computer die allerlei berekeningen kan uitvoeren \autocite{Richardson2013}. Deze bevat alle logica die uitgevoerd wordt na het scannen van de QR-code.  De aangesloten USB camera zal de afbeeldingen proberen te verwerken zodat de gekoppelde locker geopend wordt. Niet alleen het uitvoerbaar script draait op de Raspberry PI ook een server die alle verzoeken afhandelt. Indien er een QR-code goed wordt uitgelezen, stuurt het proces een verzoek naar de node.js server, die op zijn beurt de controle overneemt.
   ...
   \begin{lstlisting}[language=Python, caption={Het primaire uitvoerbare script van de QR-unit \autocite{LockitRentals}.}, label={lst:pythonPrimary2}, numbers=left]
    import cv2
...
\begin{lstlisting}[language=Python, caption={Python script om parameter code uit de link te halen \autocite{LockitRentals}.}, label={lst:pythonLinkSplitting}, numbers=left]
...
\begin{lstlisting}[language=Python, caption={Python code om parameter code te decoderen en informatie op te halen uit het object \autocite{LockitRentals}.}, label={lst:pythonLinkSplitting}, numbers=left]
...
\begin{lstlisting}[language=Python, caption={Python code om QR-code te verifieren op geldige digitale handtekening \autocite{LockitRentals}.}, label={lst:pythonVerifySigning}, numbers=left]
    router.post('/qr-code', (request, response) => {
        try {
            const decryptedData = decryptor.decrypt(request.body.tag);
            console.log(decryptedData);
...
\begin{lstlisting}[language=Python, caption={Validatie van QR-code op tijd en bijpassende QR-unit \autocite{LockitRentals}.}, label=lst:data-validator, numbers=left]
...
\begin{lstlisting}[language=Python, caption={Openen van juiste locker of foutboodschap \autocite{LockitRentals}.}, label=lst:pythonPinController, numbers=left]
    if (decryptedData.n) {
...
\begin{figure}[h]
    ...
    \captionsetup{justification=centering, singlelinecheck=false}    
    \caption{Zoek resultaten in scopus voor de termen ‘chatbot’ of ‘conversation agent’ of ‘conversational interface’ vanaf 2000 tot 2019 \autocite{Adamopoulou2020}.}
...
\begin{figure}[h]
    ...
    \captionsetup{justification=centering, singlelinecheck=false}    
    \caption{Standaard architectuur van een chatbot \autocite{Adamopoulou2020}.}
...
Chatbots kunnen geclassificeerd worden door middel van een aantal eigenschappen zoals het interactie level en op welke manier de antwoorden gegenereerd worden \autocite{Nimavat2017}.
...
\begin{figure}[h]
    ...
    \captionsetup{justification=centering, singlelinecheck=false}   
    \caption{Classificaties en soorten van een chatbot \autocite{Nimavat2017}.}
