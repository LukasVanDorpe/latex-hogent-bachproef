...
\subsection{Stappenplan om QR-code terug te winnen via chatbot}

Hieronder volgt een voorbeeld hoe je als festivalganger een QR-code zal terugwinnen via de chatbot. Deze stappen worden later gevisualiseerd in sectie \ref{sec:resultatenChatbot}
\newline
...
\subsubsection{Ophalen evenemnten waarop lockers actief zijn}
\label{ophalenActieveFestivals}
De chatbot zal de vraag stellen om een evenement aan te duiden in de chat. Een lijst van alle actieve enementen waarop de lockers actief staan is hiervoor nodig. Deze actie word uitgevoerd in een stroomdiagram aan de hand eigen geschreven code. Het antwoord op dit verzoek wordt bewaard in vooraf gedefinieerde lijsten. Deze code is voorgesteld in listing \ref{lst:botpressActie1}.
...
De ophaling van alle actieve evenementen verloopt in de backend van de Lockit Rentals applicatie. Deze worden geconverteerd naar duidelijke objecten om zo weer te geven in de chat. \ref{lst:botpressActie2}.
Methode waarin festivals worden ogpehaald waar lockers actie zijn.
...
\subsubsection{Verzoek sturen voor verloren QR-code op te halen}
\label{versturenRequestEmail}

Vervolgens zal de chat moeten vragen achter het juiste media kanaal waarin hij de QR-code wil terug geven. Bij de uitwerking van deze bachelorproef zal de communicatie lopen aan de hand van een email adres. Eénmaal de chatbot het email adres goed heeft ontvangen kan het proces van het terugwinnen van een QR-code in werking treden. De chatbot voert een taak uit dat weergegeven wordt in listing \ref{lst:botpressActie3}. 
...
\subsubsection{Ophalen van QR-code en aanmaken email }
\label{ophalenQR}

Dit verzoek zal opnieuw afgehandeld worden in de backend van de Lockit Rentals applicatie. Het uitvoeren van onderstaande code \ref{lst:botpressActie4} zal de QR-code ophalen uit de reservatie tabel. Hierbij maakt hij een antwoord waarin hij aangeeft dat de QR-code succesvol is opgehaald en de herstel email is verzonden. Op deze manier kan de chatbot een gepaste actie voorzien om de gebruiker erop te wijzen dat deze email onderweg is. 
...
\subsubsection{Versturen van verloren QR-code}
\label{versturenQRcode}

Om de aangemaakte mail met daarin de gerecupereerde QR-code te verzenden naar de eindgebruiker is echter een ander verhaal. De aangemaakte mail word bewaard in een tabel ‘customerEmail’. Om deze mail automatisch te detecteren gebruiken we de extentie binnenin Firebase genaamd ‘Tigger email’. Deze extentie zorgt voor het detecteren van email die aangemaakt zijn binnen de tabel ‘customerEmail’. De extentie kan zo nog een derde provider aanspreken genaamd ‘Mail Sender’ die ervoor zal zorgen dat de email naar de desbetreffende corresponend verstuurd word. Via deze tool kunnen we alles analayseren wat er precies met de verstuurde mail gebeurt. 
